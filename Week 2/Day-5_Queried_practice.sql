...
GROUP BY city 
HAVING total_emp >= 4 ;
-- I USE AI TO CHECK WHATS AT LEAST 4 MEAN 
...
FROM Employees
GROUP BY city ;
-- BY DOING FACT CHECKT , BOTH QUERIES GOT SAME OUTPUT , DOES MY QUERY GOES WRONG OR IT SOME LOGIC ERROR 


-- Q3 
SELECT department ,
      SUM(salary) AS total_salary ,
      AVG(experience_years) AS avg_age
FROM Employees
...
HAVING total_emp < 9 
AND avg_exp > 5;
-- DOEST SQL WORKS WITH AND CLAUSE IN HAVING CLAUSE . AND IT SHOWS NULL 49000.0000 AND SALES 50750.0000 
...
GROUP BY city 
HAVING total_emp >= 3 ;
-- I ADDED A COLUMN BEYOND THE QUERY. IS THAT FINE 
...
LIMIT 3 ; 
-- I WONT MAKE ANOHTER MISTAKE IN  AT LEAST TERMS 
...
GROUP BY department
HAVING COUNT(*) > 8 
AND min_salary < 60000; 
-- ITS KIND OF SOMETHING I CAN THINK OF , I KNOW THATS NOT THE ACTULL QUERY . THE OUTPUT | Marketing  |         9 |      36000 


-- Q9
SELECT department , ROUND(AVG(experience_years) , 1) AS avg_exp
FROM Employees 
GROUP BY department
HAVING avg_exp >= 6 
...
SELECT department , AVG(salary) AS avg_salary
FROM Employees
WHERE city <>'Bangalore' AND department <> 'Finance'
GROUP BY department
HAVING COUNT(*) >= 4
ORDER BY avg_salary DESC
LIMIT 1 ; 
-- NOT SURE ABOUT THE OUTPUT , BUT STILL FEEL LIKE ITS THE ACCURATE . OUTPUT   Engineering | 112272.7273